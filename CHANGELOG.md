...
- `core/components/update-banner/update-banner.test.js` — its `beforeEach` called `strings.js`'s `reset()` (to isolate its own `defineStrings()` fixtures per test) but never restored the registry afterward. Under vitest `isolate: false`, once this file's last test ran, `strings.js`'s shared `_locales` registry was left containing only this file's 3 fixture keys — permanently, for the rest of that worker's life — silently breaking `t()` for every real string in every test file that happened to run afterward in the same worker. This is the same bug class fixed in `core/strings.js`/`core/strings.test.js` in 0.15.9, just in a second, independent file that also called `reset()` without the matching snapshot/restore guard. Root-caused via a Telos test run where `tag-input.test.js`'s aria-label assertion failed intermittently — traced to `update-banner.test.js` running earlier in the same worker.
  ...
  Audited every other module with module-level singleton state (`core/theme/theme.js`, `modules/toast/toast.js`, `core/store/store.js`, `core/store/store-simple.js`) — none carry this risk. The bug class is specific to a module where a consuming app registers real content once via a passive import-time side effect (`app/strings.js`'s `defineStrings()` calls) that `reset()` can permanently erase; theme/toast/store have no equivalent passive registration; every test file that uses them re-establishes their own state explicitly (`boot()`, `initTheme()`, `toast()`) rather than depending on residual state from another file. Also checked `modules/toast/toast.test.js` and `modules/toast/undo.test.js`, the only other files importing anything from `strings.js`'s `reset` — both only call `defineStrings` (additive), never `reset()`, so they carry no risk either. No changes needed for any of these.
...
### Added
- `core/test-helpers.js` — new `guardSingleton(snapshot, restore)` helper wrapping the `beforeAll`/`afterAll` snapshot-restore boundary in one call, so a test file that resets a shared singleton module no longer hand-rolls the boilerplate. `core/strings.test.js` and `update-banner.test.js` both now use it instead of duplicating the pattern.
- `library_tests/singleton-reset-guard.test.js` — new automated consistency check: fails if any `core/`/`modules/` test file imports `reset` from `core/strings.js` without also importing `_snapshot`/`_restore` from it. Turns this regression class into a build-time catch instead of an intermittent test failure discovered by a consuming app.
