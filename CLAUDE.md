...
- **ARIA attributes must not linger on role-less custom element hosts.** Custom elements have no implicit ARIA role. If a component accepts `aria-label` (or `aria-labelledby`) and forwards it to an inner semantic element (e.g. `<dialog>`, `<button>`), remove the attribute from the host element immediately after forwarding. Retaining it on a role-less host is a spec violation; axe-core's `aria-prohibited-attr` rule (Lighthouse "Elements use prohibited ARIA attributes") flags every instance.
