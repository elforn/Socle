...
```
pwa-lib/
  core/               # Always scaffolded: AppElement, Router, Store, IDB, SW
  modules/
  ...
  cli/
    templates/        # Pre-generated Claude files for app developers
      .claude/
        commands/
      CLAUDE.md.template
  reference-app/      # Real app built with the library. Proves every feature.
  docs/
  .claude/
...
- **Minimal build script** (plain Node.js, no build framework) that:
  - Stamps asset filenames with a content hash
  - Generates `version.json`
  - Injects the asset manifest and version constant into `sw.js`
  - Injects `APP_VERSION` into the main JS entry point
...
- **Vitest** for unit tests. Each module in `core/` and `modules/` has a co-located `*.test.js`.
- **Playwright** for E2E. Tests live in `e2e/`. Cover: routing, data persistence across reload, SW update flow, offline behaviour.
- Test files are created alongside the feature, not after.
- IDB and Store logic must be unit-testable without a browser (use Vitest's happy-dom environment).
