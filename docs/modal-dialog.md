...
**Body swipe and nested horizontal scrolling.** A touch device's browser can claim a gesture natively from its very first sample if `touch-action` allows it — ahead of and independent from JS — and `setPointerCapture()` can't reclaim a gesture the browser already committed to. A real swipe is almost never perfectly straight, so even `touch-action: pan-y` (permitting native vertical panning while reserving horizontal for JS) isn't enough: the browser can still claim it as a vertical pan from the first sample's incidental vertical drift, before the body's own ~10px direction classification ever runs once. So for the duration of a tracked drag, the body sets `touch-action: none`, blocking native handling on both axes until JS itself has decided what the gesture is. This is set only after a drag has passed every other check (not on an interactive element, not starting inside a nested horizontally-scrollable descendant) and is released once the drag ends or is cancelled — so it never affects anything outside a single in-progress gesture.

The cost: once a gesture starts under `touch-action: none`, the browser won't hand it back to native scrolling mid-touch even after this reverts. So a drag that turns out to be vertical doesn't hand off to native scroll — the body drives its own `scrollTop` in JS for the rest of that gesture, tracking the finger 1:1 once direction is known. This costs a small (~10px) dead zone right at the start of a vertical drag, while direction is still undecided; once classified, tracking is immediate and 1:1.

A drag that starts inside an already-horizontally-scrollable descendant (detected via `scrollWidth`/`clientWidth` and computed `overflow-x`) is left alone entirely before `touch-action` is ever touched, deferring to that element's own native scrolling — a chart with its own `overflow-x: auto` region, say, keeps panning normally. A drag starting on an interactive element (`button`, `a`, `input`, `textarea`, `select`, `[contenteditable]`) is also ignored, so a `<select>` or similar inside the slotted content keeps working normally.
