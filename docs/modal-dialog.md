...
**Body swipe and native touch-action.** The body has `touch-action: none` for as long as `tabCount > 1` — a static CSS rule (`.body.has-tabs`), switched on in lockstep with the handle's own `has-tabs` class, not set reactively inside a pointer handler. That's a deliberate, hard-won choice: a value changed reactively, in response to a touch that's already begun, is not guaranteed to apply to that same gesture — a touch device's browser makes its own gesture-ownership decision on a separate thread from JS, using whatever `touch-action` was already in effect *before* a `pointerdown` handler runs, and `setPointerCapture()` can't reclaim a gesture the browser already committed to. The handle's own `touch-action: none` has always been static from initial paint and never had this problem; the body's touch-action needed to become static for the same reason.

> **This is a real, permanent behaviour change for as long as a dialog has `tabCount > 1` — not something that only applies during a swipe attempt.** CSS `touch-action` intersects down the whole ancestor chain, and a descendant can never loosen what an ancestor already restricted. So any nested horizontally-scrollable content in the body — a chart with its own `overflow-x: auto` region, say — permanently loses native panning while tabs are active, whether or not the user is mid-swipe. A consumer with that kind of content needs to implement its own manual pointer-driven horizontal scroll replication (mirroring the vertical fallback described below) — this module can't solve that generically, since it doesn't know what a consumer slots in. Whether this also affects native text-selection-via-drag on a nested `<input>`/`<textarea>` is currently **unverified** — flagged as an open question, not yet confirmed either way.

A drag starting inside an already-horizontally-scrollable descendant (detected via `scrollWidth`/`clientWidth` and computed `overflow-x`) still skips this module's own tab-swipe tracking, so a chart-scroll gesture doesn't accidentally change tabs — but this no longer restores that descendant's native panning (see above; the CSS restriction is inherited and can't be selectively lifted per-touch). A drag starting on an interactive element (`button`, `a`, `input`, `textarea`, `select`, `[contenteditable]`) is also ignored by this module's own tracking, so a `<select>` or similar inside the slotted content keeps working normally for that purpose.

The other cost: once a gesture starts under `touch-action: none`, the browser won't hand it back to native scrolling mid-touch. So a drag that turns out to be vertical doesn't hand off to native scroll — the body drives its own `scrollTop` in JS for the rest of that gesture, tracking the finger 1:1 once direction is known. This costs a small (~10px) dead zone right at the start of a vertical drag, while direction is still undecided; once classified, tracking is immediate and 1:1.
