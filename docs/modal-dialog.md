...
**Body swipe and nested horizontal scrolling.** A touch device's browser claims a gesture natively from its very first sample if `touch-action` allows it — ahead of and independent from JS, and `setPointerCapture()` can't reclaim a gesture the browser already committed to. So for the duration of a tracked drag, the body sets `touch-action: pan-y`, claiming the horizontal axis for JS while leaving vertical fully native. This is set only after a drag has passed every other check (not on an interactive element, not starting inside a nested horizontally-scrollable descendant) and is released the moment the drag ends, is cancelled, or turns out to be vertical — so it never affects anything outside a single in-progress horizontal swipe.

A drag that starts inside an already-horizontally-scrollable descendant (detected via `scrollWidth`/`clientWidth` and computed `overflow-x`) is left alone entirely before `touch-action` is ever touched, deferring to that element's own native scrolling — a chart with its own `overflow-x: auto` region, say, keeps panning normally. A drag starting on an interactive element (`button`, `a`, `input`, `textarea`, `select`, `[contenteditable]`) is also ignored, so a `<select>` or similar inside the slotted content keeps working normally. Direction itself is still disambiguated in JS from the first ~10px of pointer movement — `pan-y` only stops the browser from pre-empting that decision, it doesn't replace it.
